...
const Payment = () => {
  ...
  useEffect(() => {
    ...
    QRCode.toDataURL(JSON.stringify({
      merchant: 'PACEFORGE MOCK PAYMENTS',
      amount: due,
      currency: 'INR',
  ...
  const checkPayment = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        // Local offline fallback registration when backend server is offline or on Netlify
        const mockRegId = `PF-REG-${new Date().getFullYear()}-${Math.random().toString(36).substring(2, 10).toUpperCase()}`;
        const mockBooking = {
          ...
          status: 'CONFIRMED',
          paymentStatus: 'Mock Payment Successful',
          amount: total,
        ...
        saveMockRegistration(mockBooking);
        if (source === 'WALLET' && walletUsed > 0) {
          updateMockWalletBalance(walletUsed, 'DEBIT');
        }
        setStatus('success');
        setTimeout(() => navigate(`/booking-confirmation/${mockRegId}`), 900);
      }
    }, 1600);
  };
  ...
  return <div className="pt-28 pb-16 min-h-screen max-w-5xl mx-auto px-4">
    <ActivityAnimation category={race.category} compact />
    <p className="text-primary text-[10px] font-black uppercase tracking-[0.3em] mt-6">Secure mock checkout</p>
    ...
    <div className="grid lg:grid-cols-2 gap-8 mt-10">
      <div className="glass-card p-7">
        ...
        <div className="space-y-4 border-t border-white/10 mt-7 pt-6 text-sm">
          <div className="flex justify-between"><span className="text-gray-500">Race price</span><span>₹{amount.toLocaleString()}</span></div>
          <div className="flex justify-between"><span className="text-gray-500">GST (18%)</span><span>₹{tax.toLocaleString()}</span></div>
          <div className="flex justify-between"><span className="text-gray-500">Processing charge</span><span>₹{extra.toLocaleString()}</span></div>
          <div className="flex justify-between text-xl font-black border-t border-white/10 pt-5"><span>Total</span><span className="text-primary">₹{total.toLocaleString()}</span></div>
        </div>
        <div className="mt-7 p-4 bg-white/5 rounded-md flex items-center justify-between">
        ...
        <p className="text-xs text-gray-500 mt-3">Wallet used: ₹{walletUsed.toLocaleString()} · External amount: ₹{due.toLocaleString()}</p>
      </div>
      <div className="glass-card p-7">
        <h2 className="text-2xl font-black uppercase italic">Payment source</h2>
        <div className="grid grid-cols-2 gap-3 mt-6">
          {['WALLET', 'UPI', 'CREDIT_CARD', 'DEBIT_CARD', 'NET_BANKING'].map(item => (
            <button key={item} onClick={() => setSource(item)} className={`p-4 border rounded-md text-[10px] font-black uppercase tracking-widest ${source === item ? 'border-primary bg-primary/10 text-primary' : 'border-white/10 text-gray-500'}`}>
        ...
        <button onClick={checkPayment} disabled={status !== 'idle'} className="hero-button w-full mt-6 py-4 flex items-center justify-center gap-2">
          {status === 'checking' ? <><LoaderCircle className="animate-spin" size={17}/> Checking payment...</> : status === 'success' ? <><CheckCircle2 size={17}/> Payment successful</> : <><CreditCard size={17}/> Check payment status</>}
        </button>
        {qr && <div className="mt-7 text-center">
          <p className="text-[10px] font-black uppercase tracking-widest text-gray-500">Scan mock payment QR · ₹{due.toLocaleString()}</p>
          <img src={qr} alt="Mock payment QR code" className={`w-52 h-52 mx-auto mt-3 bg-white p-2 ${status === 'checking' ? 'animate-pulse' : ''}`}/>
        </div>}
        {error && <p className="text-primary text-xs mt-5">{error}</p>}
        <p className="text-gray-600 text-xs mt-7">Mock only. This QR encodes the amount and reference; it does not transfer money.</p>
      </div>
    </div>
  </div>;
};
